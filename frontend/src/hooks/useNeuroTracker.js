...
export function useNeuroTracker() {
  ...
  // Adaptive batching: longer on analytics page
  const BATCH_INTERVAL = location.pathname.includes('analytics') ? 2000 : 1000;

  // ========== BUFFER FLUSHING ==========
  const flushBuffer = useCallback(() => {
    if (actionBufferRef.current.length > 0) {
      // Only log on non-analytics pages to reduce noise
  ...
  }, [sendBatchActions, location.pathname]);
  ...
  const trackAction = useCallback(
    (type, elementId = null, targetPath = null) => {
      ...
      // Only log click and scroll events (reduce console spam for hover)
      if (type !== "HOVER") {
